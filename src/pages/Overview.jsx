// src/pages/Overview.js (FULL UPDATED FILE — IMPROVED DEFAULT PAGE TEXT AND CONTENT)
...
          // 🔑 Use the correct, secure API endpoint for analysis sessions
          `${API}/api/analysis/sessions`, 
          {
            headers: {
              // Send the JWT token for user-specific data retrieval
...
          // Assuming the backend returns the most current session first (or we pick the first one)
          const current = data.sessions[0]; 
          // The session data (kpis, analysisText, etc.) is stored in 'session_data'
          setDashboard(current.session_data ? JSON.parse(current.session_data) : null);
        } else {
          setDashboard(null); // No data found, show default screen
        }
      } catch (err) {
