...
export const Visualizer = ({
  ...
  useEffect(() => {
    if (
      ...
      !activeDatasets.some((ds) => ds.id === selectedDatasetId)
    ) {
  ...
  const parsed = useMemo(() => {
    return activeDatasets.map((ds) => {
      const columns = Array.isArray(ds.data?.[0])
        ? ds.data[0]
      ...
      const labelColName = labelCol ? String(labelCol) : "";

      const isDateLabel =
        labelColName.toLowerCase().includes("date") ||
