 ...
 * FIX: Accumulative selection logic to prevent overwriting previous picks.
 */
import React, { useState } from "react";
import { FiX, FiUploadCloud, FiDatabase, FiFileText, FiCheck, FiAlertTriangle, FiLoader, FiPlus, FiTrash2 } from "react-icons/fi";
...
export const ImportModal = ({ 
    ...
    // Core change: We maintain the list locally to ensure "Append" logic works
            ...
            const picker = new window.google.picker.PickerBuilder()
                ...
                .setCallback((data) => {
                    if (data.action === window.google.picker.Action.PICKED) {
                        ...
                        // FIX: Filter out duplicates and APPEND to existing state
                        setSheetIds(prev => {
                        ...
                        setSheetNames(prev => {
                            ...
                            // We sync the comma-separated string back to the parent for the UI
    ...
    // ... handleDrag, handleDrop, handleFileChange logic remains same ...
    const handleFileChange = (e) => { if (e.target.files && e.target.files[0]) setCsvToImport(e.target.files[0]); };
    const handleDrag = (e) => { e.preventDefault(); e.stopPropagation(); if (e.type === "dragenter" || e.type === "dragover") setDragActive(true); else if (e.type === "dragleave") setDragActive(false); };
    const handleDrop = (e) => { e.preventDefault(); e.stopPropagation(); setDragActive(false); if (e.dataTransfer.files && e.dataTransfer.files[0]) setCsvToImport(e.dataTransfer.files[0]); };
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-[#050505] border border-white/10 rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 max-h-[90vh]">
                ...
                <div className="p-10 bg-white/[0.01] border-t border-white/5 mt-auto">
                    ...
                    >
                        <FiDatabase size={20} />
                        Import {sheetIds.length} Neural Streams
                    </button>
                </div>
            </div>
        </div>
